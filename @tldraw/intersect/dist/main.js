...
// src/index.ts
// var {Vec} = Vec2
var {lrp, isEqual ,angle, sub, rotWith, slope, add, mul} = Vec
...
function intersectLineLine(AB, PQ) {
  const slopeAB = slope(AB[0], AB[1]);
  const slopePQ = slope(PQ[0], PQ[1]);
  if (slopeAB === slopePQ)
    return void 0;
...
function intersectRayEllipse(origin, direction, center, rx, ry, rotation) {
  const a1 = origin;
  const a2 = mul(direction, 999999999);
...
function intersectLineSegmentLineSegment(a1, a2, b1, b2) {
  ...
  if (u_b !== 0) {
    ...
    if (0 <= ua && ua <= 1 && 0 <= ub && ub <= 1) {
      return createIntersection("intersection", add(a1, mul(AV, ua)));
    }
  }
  return createIntersection("no intersection");
}
...
function intersectLineSegmentArc(a1, a2, center, radius, start, end) {
  const sa = angle(center, start);
  const ea = angle(center, end);
  const ellipseTest = intersectEllipseLineSegment(center, radius, radius, 0, a1, a2);
  if (!ellipseTest.didIntersect)
    return createIntersection("no intersection");
  const points = ellipseTest.points.filter((point) => isAngleBetween(sa, ea, angle(center, point)));
  if (points.length === 0) {
    return createIntersection("no intersection");
  }
...
function intersectLineSegmentCircle(a1, a2, c, r) {
  ...
  const results = [];
  if (0 <= u1 && u1 <= 1)
    results.push(lrp(a1, a2, u1));
  if (0 <= u2 && u2 <= 1)
    results.push(lrp(a1, a2, u2));
  return createIntersection("intersection", ...results);
}
function intersectLineSegmentEllipse(a1, a2, center, rx, ry, rotation = 0) {
  if (rx === 0 || ry === 0 || isEqual(a1, a2)) {
    return createIntersection("no intersection");
  }
  rx = rx < 0 ? rx : -rx;
  ry = ry < 0 ? ry : -ry;
  a1 = sub(rotWith(a1, center, -rotation), center);
  a2 = sub(rotWith(a2, center, -rotation), center);
  ...
  const points = tValues.filter((t) => t >= 0 && t <= 1).map((t) => add(center, add(a1, mul(sub(a2, a1), t)))).map((p) => rotWith(p, center, rotation));
  return createIntersection("intersection", ...points);
}
...
function intersectRayPolygon(origin, direction, points) {
  const sideIntersections = pointsToLineSegments(points, true).reduce((acc, [a1, a2], i) => {
    const intersection = intersectRayLineSegment(origin, direction, a1, a2);
    if (intersection) {
...
function pointsToLineSegments(points, closed = false) {
  const segments = [];
  for (let i = 1; i < points.length; i++)
